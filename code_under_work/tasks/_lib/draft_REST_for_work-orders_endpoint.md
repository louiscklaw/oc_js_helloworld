...
1. create a Node.js script using `axios` or `node-fetch` to test all work-orders CRUD operations:
   1. GET all work-orders (list endpoint)
   1. GET work-order by ID (single item endpoint)
   1. POST create new work-order
   1. PUT update existing work-order
   1. DELETE work-order by ID
1. create comprehensive test scenarios including:
   1. valid data tests
   1. invalid data tests
   1. edge cases (empty responses, non-existent IDs)
   1. error handling tests
1. update the existing `test.http` file with additional work-order endpoint test cases
