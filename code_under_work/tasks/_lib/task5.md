...
1. integrate `jest` test tools and add a helloworld test to the `dashboard` page.
  1. update the `AGENTS.md` to let the decendnt AI knows how to use it
  1. update the `package.json` for `jest` test command
  1. run yourself the test command(s) created.
