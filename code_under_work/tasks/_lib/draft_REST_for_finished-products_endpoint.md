...
1. create a Node.js script using `axios` or `node-fetch` to test all finished-products CRUD operations:
   1. GET all finished-products (list endpoint)
   1. GET finished-product by ID (single item endpoint)
   1. POST create new finished-product
   1. PUT update existing finished-product
   1. DELETE finished-product by ID
1. create comprehensive test scenarios including:
   1. valid data tests
   1. invalid data tests
   1. edge cases (empty responses, non-existent IDs)
   1. error handling tests
1. update the existing `test.http` file with additional finished-product endpoint test cases
