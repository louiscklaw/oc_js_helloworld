...
code about `customer` and `customers` is known good and can be regards as a template.
code `warehouse` and `employee` is migrated code from `customer` and `customers`. You may found faulty.
please read `AGENTS.md` for more detail.
endpoint directory for CRUD were located in `src/app/api/db/<model>`
schema is managed by `prisma/schema.prisma`
...
1. i want you to review the source code of `dashboard/finished-products` and `components/finished-products`. you may still find some vairable(s)/function(s) named with `customer` or `customers`. please help to rename it.
1. i want you to review the source code of `src/app/api/db/finished-product` endpoint. you may still find some vairable(s)/function(s) named with `customer` or `customers`. please help to rename it.
1. i want you to review the source code of `src/db/controllers/finished-product` and `src/db/models/finished-product`. you may still find some vairable(s)/function(s) named with `customer` or `customers`. please help to rename it.
1. i want you to review the source code of `src/db/types/finished-product.d.ts`. you may still find some vairable(s)/function(s) named with `customer` or `customers`. please help to rename it.
