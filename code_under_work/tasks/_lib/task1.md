...
imagine you are on the project root directory,
...
1. please run `pnpm run app_AI_code_selfcheck` and check the result for error(s) and write it down in `./.tmp/AI_TODO.md`
1. solve the error in `./.tmp/AI_TODO.md`.
1. please review `AGENTS.md` and trim it down. (The target audience is AI Agents and let them to easy to understand)
1. please review `.vscode/settings.json` and append the file(s) to ignore into the `exclude` list.
