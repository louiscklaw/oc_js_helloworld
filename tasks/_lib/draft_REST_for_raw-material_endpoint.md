...
1. create a Node.js script using `axios` or `node-fetch` to test all raw-material CRUD operations:
   1. GET all raw-materials (list endpoint)
   1. GET raw-material by ID (single item endpoint)
   1. POST create new raw-material
   1. PUT update existing raw-material
   1. DELETE raw-material by ID
