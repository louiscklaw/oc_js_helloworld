...
1. plesae help to draft a new endpoint `work-order` with reference to the current endpoints
  1. update the `test.http` to make a sanity test of the endpoint.
  1. create a nodejs simple script with `request` to test the endpoint(s) CRUD operations
1. plesae help to draft a new endpoint `raw-material` with reference to the current endpoints (i.e. `warehouse`, `employee`)
  1. update the `test.http` to make a sanity test of the endpoint.
  1. create a nodejs simple script with `request` to test the endpoint(s) CRUD operations
